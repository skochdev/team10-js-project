...
export async function fetchKeyWord(searchQuery, page = 1) {
  onLoaderVisible();
  const { data } = await axios(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&language=en-US&query=${searchQuery}&page=${page}`);
  return data;
}
...
export async function fetchVideoByMovieId(movie_id) {
